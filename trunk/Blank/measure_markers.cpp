...
std::vector<cv::Point2f> circleDetect(cv::Mat img_gray) {
  ...
  std::vector<cv::Point2f> corners, result, ans;
  ...
  */
  ...
  for (size_t i = 0; i != corners.size(); ++i) {
    ...
    for (int k = 10; k<15 && judge; k++) {
	  ...
	  int black[4] = {0};
	  black[3] = blacknum[3]+blacknum[0];
	  for(int i=0; i<3; i++){
		black[i] = blacknum[i]+blacknum[i+1];
	  }
	  for(int i=0; i<4; i++){
		if((double)black[i]*2/num < 0.2 || (double)black[i]*2/num >0.7){
			judge=0;
			break;
		}
	  }
	  if(abs(black[0]-black[2])>3 || abs(black[1]-black[3])>3){
		judge = 0;
		break;
	  }

      if (judge == 0 || reverse_time1 == 0 || reverse_time2 == 0 || reverse_time1 + reverse_time2<4){
        break;
      }
...
void MeasureMarkersWindow::reconstruction(){
	mStatusBar->showMessage(QString::fromWCharArray(L"测试窗口"));
}
//捕获一张图像
...
void MeasureMarkersWindow::createWidget() {
  ...
  mRecon = new QPushButton();
  mRecon->setText(QString::fromWCharArray(L"重建"));
  connect(mRecon, SIGNAL(pressed()), this, SLOT(reconstruction()));
  mRecon->setDisabled(true);
