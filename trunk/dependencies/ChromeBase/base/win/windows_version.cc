...
void _FixWindowsVersion(OSVERSIONINFOEX &version_info)
{
  HMODULE hkernel32 = GetModuleHandle(L"kernel32.dll");
  if (hkernel32)
  {
    FileVersionInfo *version_kernel32 = FileVersionInfo::CreateFileVersionInfoForModule(hkernel32);
    if (version_kernel32)
    {
      int v1 = 0, v2 = 0, v3 = 0, v4 = 0;
      base::string16 str_file_version = version_kernel32->file_version();
      if (4 == swscanf(str_file_version.c_str(), L"%d.%d.%d.%d", &v1, &v2, &v3, &v4) && 6 == v1)
      {
        version_info.dwMajorVersion = v1;
        version_info.dwMinorVersion = v2;
        version_info.dwBuildNumber = v3;
      }
    }
  }
}
...
OSInfo::OSInfo()
  ...
  // ksbrowser core {
  // fix version use kernel32.dll 
  _FixWindowsVersion(version_info);
  // }
