...
SetGpuInfoFuncT g_set_gpu_info_func = NULL;

void ksSetGpuInfoReprotingFunction(SetGpuInfoFuncT set_gpu_info_func){
  g_set_gpu_info_func = set_gpu_info_func;
}

void ksSetGpuInfo(const base::StringPiece& vendor_id, const base::StringPiece& device_id,
  const base::StringPiece& driver_version, const base::StringPiece& pixel_shader_version,
  const base::StringPiece& vertex_shader_version){

  if (!g_set_gpu_info_func)
    return;

  g_set_gpu_info_func(vendor_id, device_id, driver_version, pixel_shader_version, vertex_shader_version);
}


SetExtensionIDT g_set_extensionid_func = NULL;
void ksSetExtensionIDInfoFunction(SetExtensionIDT set_extension_id_func){
  g_set_extensionid_func = set_extension_id_func;
}
void ksSetExtensionID(int index, const base::StringPiece& id){
  if (!g_set_extensionid_func)
    return;
  g_set_extensionid_func(index,id);
}


SetNumberOfExtensionsT g_set_number_of_extensions_func = NULL;
void ksSetNumberOfExtensionFunction(SetNumberOfExtensionsT set_extentsion_num_func){
  g_set_number_of_extensions_func = set_extentsion_num_func;
}
void ksSetNumberOfExtensions(int number_of_extensions){
  if (!g_set_number_of_extensions_func)
    return;
  g_set_number_of_extensions_func(number_of_extensions);
}



SerCommandlineInfoT g_set_commandline_func = NULL;
void ksSetCommandLineInfoFunction(SerCommandlineInfoT set_commandline_func){
  g_set_commandline_func = set_commandline_func;
}
void ksSetCommandLine(const wchar_t** argv, size_t argc){

  if (!g_set_commandline_func)
    return;

  g_set_commandline_func(argv, argc);
