...
// ksbrowser {
#include "ksmodule/kbase/allocator/allocator.h"

namespace kbase {
  static ALLOCATOR allocator_;
  void setallocator(const ALLOCATOR* alloc)
  {
    memcpy(&allocator_, alloc, sizeof (ALLOCATOR));
  }
...
typedef enum {
  ...
  //ksbrowser
  KS_ALLOC,     // chrome.dll和liebao.exe使用同一个堆
  //
} Allocator;
...
void* malloc(size_t size) {
  ...
  for (;;) {
    switch (allocator) {
      ...
      // ksbrowser {
      case KS_ALLOC:
        ptr = kbase::getallocator()->xmalloc(size);
        break;
      // }
      case TCMALLOC:
      default:
...
void* realloc(void* ptr, size_t size) {
  ...
  for (;;) {
    switch (allocator) {
      ...
      // ksbrowser {
      case KS_ALLOC:
        new_ptr = kbase::getallocator()->xrealloc(ptr, size);
        break;
      // }
      case TCMALLOC:
      default:
...
extern "C" size_t _msize(void* p) {
  switch (allocator) {
    ...
    // ksbrowser {
    case KS_ALLOC:
      return kbase::getallocator()->xmsize(p);
    // }
  }
...
// implementation requires Winheap to be the allocator.
  ...
  // ksbrowser {
  // 设置 内存分配器 为 ks_alloc
  if ( kbase::getallocator()->xmalloc != NULL ) {
    allocator = KS_ALLOC;
    return 1;
  }
  // }
  ...
  for (;;) {
    switch (allocator) {
      ...
      case WINLFH:
        ...
        // ksbrowser {
      case KS_ALLOC:
        ptr = kbase::getallocator()->xaligned_malloc(size, alignment);
        break;
        // }
...
void _aligned_free(void* p) {
  ...
  switch (allocator) {
    ...
    case WINLFH:
      ...
      // ksbrowser {
    case KS_ALLOC:
      kbase::getallocator()->xaligned_free(p);
      return;
      // }
