...
int UsbCamera::g_result1 = 1;
int UsbCamera::g_result2 = 1;
unsigned char* UsbCamera::g_buffer0 = NULL;
unsigned char* UsbCamera::g_buffer1 = NULL;

//这些全局变量，用于测试函数执行时间
Timer UsbCamera::gTimer;
std::vector<int> UsbCamera::gAllTimes;;
int UsbCamera::gt11,UsbCamera::gt12,UsbCamera::gt21,UsbCamera::gt22;
...
UsbCamera::UsbCamera(std::string config)
{
	//初始化所有设备
...
//初始化所有的摄像机
std::vector<CameraInfos> UsbCamera::iniCameraInfos(std::string config)
{
	//获得相机个数
	int count = 0;
...
int UsbCamera::getCameraIndex(int id)
{
...
//返回索引指定的摄像机id
int UsbCamera::getCameraId(int index)
{
	for(int i=0;i<mCamInfo.size();i++)
	{
...
//返回索引指定的摄像机id
std::string UsbCamera::getCameraSn(int index)
{
	for(int i=0;i<mCamInfo.size();i++)
	{
		if(mCamInfo[i].index == index)
...
//返回系统中可以使用的摄像机的个数
int UsbCamera::getCameraCount()
{
	return mCamNum;
}
//停止摄像机
void UsbCamera::stopCamera(int index)
{
...
bool UsbCamera::setGain(int id,int gain)
{
	int index =getCameraIndex(id);
	if(index == -1)
	{
...
bool UsbCamera::setGainAll(int gain)
{
	for(int i=0;i<mCamNum;i++)
	{
...
void UsbCamera::setAutoGain(bool t)
{
	for(int i=0;i<mCamNum;i++)
	{
		CameraSetAGC(i,t);
...
//读写第i个编号摄像头的曝光
int UsbCamera::getExposure(int id)
{
	int index =getCameraIndex(id);
	if(index == -1)
	{
...
bool UsbCamera::setExposure(int id,int expo)
{
	int index =getCameraIndex(id);
	if(index == -1)
	{
...
bool UsbCamera::setExposureAll(int expo)
{
	for(int i=0;i<mCamNum;i++)
	{
...
void UsbCamera::setAutoExposure(bool t)
{
	for(int i=0;i<mCamNum;i++)
	{
		CameraSetAEC(i,t);
...
void UsbCamera::stopOneCamera(int id)
{
	int index = getCameraIndex(id);
	if(index !=-1)
		CameraStop(index);
...
void UsbCamera::startOneCamera(int id)
{
	int index = getCameraIndex(id);
	if(index !=-1)
...
/*******************************************************捕获函数*******************************************/
//读取特定摄像头的一帧图像
bool UsbCamera::captureOneFrame(int id)
{
	int index =getCameraIndex(id);
	if(index == -1)
	{
...
//读取所有摄像头的图像
bool UsbCamera::captureOneFrameAll()
{
	bool ret = true;
	for(int i=0;i<mCamNum;i++)
	{
...
//捕获2张图像
bool UsbCamera::captureTwoFrame(int id1,int id2)
{
...
//返回捕获的缓存，参数为id
unsigned char* UsbCamera::getBuffer(int id)
{
	int index = getCameraIndex(id);
	return buffers[index];
}
unsigned char* UsbCamera::getBufferByIndex(int index)
{
	return buffers[index];
}
cv::Mat UsbCamera::getImage(int id)
{
	int index = getCameraIndex(id);
	return cv::Mat(mHeight,mWidth,CV_8UC1,buffers[index]);
}
cv::Mat UsbCamera::getImageByIndex(int index)
{
	return cv::Mat(mHeight,mWidth,CV_8UC1,buffers[index]);
}

//测试有问题,这个版本中需要把CAMERA_IMAGE_RAW8改为CAMERA_IMAGE_GRAY8
bool UsbCamera::captureOneCameraWithTrigger()
{
	//读取数据，记录到对应的缓存中
...
bool UsbCamera::captureOneCameraWithoutTrigger()
{
	//读取数据，记录到对应的缓存中
...
/*******************************************************同步捕获*******************************************/
void UsbCamera::setTriggerMode(bool t)
{
	if(t)
	{
		for(int i=0;i<mCamNum;i++)
...
bool UsbCamera::captureTwoFrameSyncSoftware(int n1,int n2)
{
...
bool UsbCamera::captureTwoFrameSyncHardware(int n1,int n2)
{
	int index1 =getCameraIndex(n1);
	if(index1 == -1)
	{
